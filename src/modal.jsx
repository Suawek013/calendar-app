    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-head">
          <span className="modal-kicker">{isNew ? "New block" : "Edit block"}</span>
          <button className="icon-btn" onClick={onClose}><Icon name="x" size={18} /></button>
        </div>

        <input className="modal-label" placeholder="What is it?" value={b.label}
          autoFocus onChange={(e) => set("label", e.target.value)} />
        <input className="modal-sub" placeholder="Add a note (optional)" value={b.sublabel || ""}
        ...
        <div className="modal-row">
          <div style={{ flex: 1 }}>
            <div className="modal-section-label">Start</div>
          ...
          <div style={{ flex: 1 }}>
            <div className="modal-section-label">End</div>
        ...
        <div className="modal-section-label">Repeat</div>
        <Segmented value={b.repeat || "this"} onChange={(v) => set("repeat", v)}
          options={[{value:"this",label:"This week"},{value:"every",label:"Every week"},{value:"custom",label:"Custom days"}]} />
        ...
        <div className="status-row">
          {[["planned","Planned"],["done","Done"],["skipped","Skipped"]].map(([s, lbl]) => (
        ...
        <div className="modal-actions">
          {!isNew && <button className="del-btn" onClick={() => onDelete(b.id)}><Icon name="trash" size={15} /> Delete</button>}
          <div style={{ flex: 1 }} />
